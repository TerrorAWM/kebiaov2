...
- **周导航增强**：支持上一周/下一周连续切换，显示动态周区间（`M.D - M.D`）
- **分享权限控制**：分享链接支持“是否允许跨周查看”并可在管理页实时修改
- **主题模式**：支持 `自动 / 亮色 / 深色`，手动选择可跨页面缓存
- **高度自定义**：支持自定义课表时间、周数，可自由选择显示字段
- **现代化 UI**：响应式设计，含手机端深度优化
...
如果你是从旧版本升级，需要运行迁移脚本检查并修复数据库缺失结构：

1. 确保 `config.php` 配置正确
2. 浏览器访问：`http://你的域名/migrate.php`
3. 使用 BACKUP 密码登录迁移页面
4. 先查看缺失项检测结果，再点击修复
5. 完成后建议删除 `migrate.php` 文件
...
| 版本 | 日期 | 说明 |
| :--- | :--- | :--- |
| **v2.1.2** | 2026-04-02 | 周导航/分享跨周权限/主题模式/移动端深度优化 |
| **v2.1.1** | 2026-04-01 | 修复共享页数据库前缀问题 |
